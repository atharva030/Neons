...
const AddTask = props => {
  ...
  const validateForm = () => {
    if (taskName.trim().length < 6) {
      Alert.alert(
        'Invalid Task Name',
        'Task Name must be at least 6 characters long.',
      );
      return false;
    }
  ...
  const addTaskdb = () => {
    ...
    if (taskName.trim() !== '') {
      if (
        new Date(stDate) > yesterday &&
        new Date(endDate) > new Date(stDate)
      ) {
        // Sample API call
        ...
        fetch(
          `https://tsk-final-backend.vercel.app/api/task/${teamIdByItem}/tasks`,
          {
            method: 'PATCH',
            headers: {
